...
# Page configuration
st.set_page_config(
    page_title="≡ƒÄ« Event Tracker - Gamified Scoring System",
    page_icon="≡ƒÄ«",
    layout="wide",
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">≡ƒÄ« Event Tracker - Gamified Scoring System</h1>', unsafe_allow_html=True)
    ...
    if st.session_state["authentication_status"] is False:
        ...
        with col2:
            st.subheader("≡ƒô¥ New User Registration")
            with st.form("registration_form"):
                ...
                if st.form_submit_button("Register ≡ƒÜÇ"):
        ...
        # User is logged in
        st.success(f'Welcome *{st.session_state["name"]}*! ≡ƒÄë')
        ...
        if st.session_state['is_admin']:
            # Admin interface
            tabs = st.tabs(["≡ƒÅá Dashboard", "≡ƒÅå Leaderboard", "ΓÜÖ∩╕Å Admin Panel", "≡ƒôº Email Center"])
            ...
            tabs = st.tabs(["≡ƒÅá Dashboard", "≡ƒÅå Leaderboard"])
...
def show_leaderboard(db):
    ...
    if not scores_df.empty:
        ...
        st.info("No scores available yet. Check back after the games begin! ≡ƒÄ«")

def show_email_center(db, email_service):
    """Display email center for admins"""
    st.subheader("≡ƒôº Email Center")
    ...
    with col1:
        ...
        st.subheader("≡ƒô¥ Email Template")
        st.text_area(
            "Email Body (Use {name}, {total_score}, {gift_type} as placeholders)",
            value="""Dear {name},

Congratulations on participating in our exciting event! ≡ƒÄë
...
Event Team""",
    ...
    with col2:
        st.subheader("≡ƒôè Email Preview")
        if st.button("Preview Email"):
            st.info("Email preview will be shown here")
        
        if st.button("≡ƒÜÇ Send Emails", type="primary"):
            with st.spinner("Sending emails..."):
                try:
                    ...
                    if not recipients.empty:
                        ...
                        st.success(f"Γ£à Successfully sent {success_count} emails!")
                    else:
